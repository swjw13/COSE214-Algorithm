...
int read_chars(FILE *fp, int *ch_freq)		//ch_freq 는 길이 256의 int형 배열
{
	char tmp;
	int num;
	int total = 0;
...
{
	int i;
	HEAP *new_heap = heapCreate(256);
	tNode *new_node;
	tNode *min_node1;
	tNode *min_node2;
	tNode *hap_node;
	for (i = 0; i < 256; i++)			//개별 알파벳에 대한 노드를 heap에 넣어줌
	{
		new_node = newNode(i, ch_freq[i]);
		heapInsert(new_heap, new_node);			//heapInsert의 return 값은 1
	}

	while (new_heap->last > 0)		//heap의 last값이 0이 되면 종료
	{
		min_node1 = heapDelete(new_heap);
		min_node2 = heapDelete(new_heap);

		hap_node = newNode(0, ((min_node1->freq) + (min_node2->freq)));		//새로운 node의 data는 0으로 지정, 두 node의 freq합을 freq으로 가짐
		hap_node->left = min_node1;
		hap_node->right = min_node2;					//new_node의 subtree들을 새로 지정해줌. 방법에 대해 다시 간구해야함

		heapInsert(new_heap, hap_node);
	}
	tNode *res = heapDelete(new_heap);
...
int encoding(char *codes[], FILE *infp, FILE *outfp)
{
	...
	while (1)
	{
		...
		{
			fprintf(outfp, "%s", codes[num]);
			total += strlen(codes[num]);
		}
	}
	return total / 8;
}
...
void decoding(tNode *root, FILE *infp, FILE *outfp)
{
	char tmp;
	tNode *check = root;
	while (fscanf(infp, "%c", &tmp) != EOF)
	{
		if (tmp == '0')
		{
			if (check->left->left == NULL && check->left->right == NULL)
			{
				fprintf(outfp, "%c", check->left->data);
				check = root;
			}
			else
			{
		...
		{
			if (check->right->right == NULL && check->right->left == NULL)
			{
				fprintf(outfp, "%c", check->right->data);
				check = root;
			}
			else
			{
